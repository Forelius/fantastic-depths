export class fadeCombatant extends Combatant {
    ...
    * @param {Partial<any>} data           Initial data from which to construct the Combatant
    * @param {any} context   Construction context options
    */
   ...
   get declaredManuever() {
      return this.actor.system.combat?.declaredAction
         ? CONFIG.FADE.CombatManeuvers[this.actor.system.combat.declaredAction]
         : null;
   }
   /** The phase that their declared action occurs in, or null. */
   get declaredActionPhase() {
      return this.declaredManuever?.phase;
   }
   /** This is only valid when using multi-phase combat sequence with a swifter action phase. */
   get isSwifterAction() {
      return this.declaredActionPhase && CONFIG.FADE.CombatPhases.immortal.swifter[this.declaredActionPhase] !== undefined;
   }
   get attacks() {
   ...
   get canMove() {
      return this.declaredManuever?.canMove === true;
   }
   get canChangeAction() {
      return game.user.isGM === true || this.initiative === null;
   }
   get availableActions() {
      if (this.initiative == null || Object.entries(this._availableActions).length == 0) {

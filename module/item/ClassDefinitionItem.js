...
export class ClassDefinitionItem extends FDItem {
    ...
    async #updateLevels(updateData) {
        ...
        if (currentFirstLevel === undefined) {
            // Insert
            levels.unshift(...Array.from({ length: newMaxLevel - newFirstLevel }, (_, index) => { return { level: index + newFirstLevel }; }));
        }
        else if (newFirstLevel < currentFirstLevel) {
            // Insert
            levels.unshift(...Array.from({ length: currentFirstLevel - newFirstLevel }, (_, index) => {
        ...
        if (newMaxLevel < currentMaxLevel) {
            ...
            levels.push(...Array.from({ length: newMaxLevel - currentMaxLevel }, (_, index) => {

...
//   static xpTable = {
//      "0.99": { baseXP: 5, bonus: 1 }, "1": { baseXP: 10, bonus: 3 }, "1+": { baseXP: 15, bonus: 4 },
...
//      const effectiveHitDice = hitDiceString.includes("+")
//         ? hitDiceString // Keep the "+", map to the correct table entry
//         : hitDiceString.includes("-")
//            ? hitDiceString.split("-")[0]
//            : hitDiceString;

//      const numericHitDice = parseInt(effectiveHitDice);

//      // Check within XP Table ranges
//      for (const [key, { baseXP, bonus }] of Object.entries(this.xpTable)) {
//         const [low, high] = key.includes("+")
//               ? [parseInt(key), parseInt(key) + 1]
//               : [parseInt(key), parseInt(key)];
//         if (numericHitDice >= low && numericHitDice <= high || key === effectiveHitDice) {
//            // Ensure we correctly map entries like "10+" to "11" if needed
//            if (key.includes("+") && !this.xpTable[effectiveHitDice]) {
//               const nextKey = (numericHitDice + 1).toString();
//               if (this.xpTable[nextKey]) {
//                  return this.xpTable[nextKey].baseXP + (this.xpTable[nextKey].bonus * asteriskValue);
//               }
//            }
//            return baseXP + (bonus * asteriskValue);
//         }
//      }
...
//      throw new Error(`Invalid Hit Dice value: '${hitDiceString}' with asteriskValue: ${asteriskValue}`);
//   }
//}
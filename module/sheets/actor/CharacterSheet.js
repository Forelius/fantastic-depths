...
export class CharacterSheet extends CharacterSheetBase {
    ...
    */
    ...
    _onClickTab(event) {
        ...
        if (!tab || button.classList.contains("active") || (event.button !== 0))
            return;
    ...
    changeTab(tab, group, { event, navElement, force = false, updatePosition = true } = { event: null, navElement: null }) {
        ...
        if ((this.tabGroups[group] === tab) && !force)
            return; // No change necessary
        const tabElement = this.element.querySelector(`.tabs > [data-group="${group}"][data-tab="${tab}"]`);
        if (!tabElement)
            throw new Error(`No matching tab element found for group "${group}" and tab "${tab}"`);
        ...
        if (this.options.position.width === "auto")
            positionUpdate.width = "auto";
        if (this.options.position.height === "auto")
            positionUpdate.height = "auto";

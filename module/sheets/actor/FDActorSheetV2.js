...
export class FDActorSheetV2 extends DragDropMixin(HandlebarsApplicationMixin(ActorSheetV2)) {
        ...
        itemContainers.forEach(containerInfo => {
            ...
            itemNames.forEach(itemNameElement => {
                ...
                if (itemId) {
                    ...
                    if (chargesElement)
                        chargesElement.style.display = isVisible ? '' : 'none';
                    if (usesElement)
                        usesElement.style.display = isVisible ? '' : 'none';
                }
                if (isVisible)
                    visibleItems++;
            });
    ...
    async _prepareContext(options) {
        ...
        context.effects = EffectManager.prepareActiveEffectCategories(this.actor.allApplicableEffects());
        // Equipped Weapons
        const attackGroups = [];
    ...
    static async #toggleContent(sheet, parent) {
        ...
        if (!collapsibleItems || collapsibleItems.length == 0)
            return;
